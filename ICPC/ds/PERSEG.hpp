struct Node {
	...
	Node(const Node& l, const Node& r) : id(-1), lid(l.id), rid(r.id), sum(0) {
		if(l.id != -1) sum += l.sum;
		if(r.id != -1) sum += r.sum;
	}
};
...
int upd(int id, int i, int l, int r) {
	...
	int mid = (l + r) >> 1;
	int lid, rid;
	if(i <= mid) lid = upd(seg[id].lid, i, l, mid), rid = seg[id].rid;
	else lid = seg[id].lid, rid = upd(seg[id].rid, i, mid + 1, r);
	seg.push_back(Node(seg[lid], seg[rid]));
	seg.back().id = isz(seg) - 1;
	return seg.back().id;
}
...
int get_kth(int idl, int idr, int k, int l, int r) {
	...
	if(cnt >= k) return get_kth(seg[idl].lid, seg[idr].lid, k, l, mid);
	return get_kth(seg[idl].rid, seg[idr].rid, k - cnt, mid + 1, r);
}
