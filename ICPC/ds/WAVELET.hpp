...
void build(int id = 0) {
	for(int i = 1; i < isz(wavelet[id].val); ++i) {
		wavelet[id].mn = min(wavelet[id].mn, wavelet[id].val[i]);
		wavelet[id].mx = max(wavelet[id].mx, wavelet[id].val[i]);
	}
	...
	if(isz(wavelet[id].val) <= 2 || wavelet[id].mn == wavelet[id].mx) return;
	if(isz(l.val) > 1) {
...
int get_kth(int id, int k, int l, int r) {
	...
	if(wavelet[id].mn == wavelet[id].mx) return wavelet[id].mn;
	int cnt = wavelet[id].cnt[r] - wavelet[id].cnt[l - 1];
	if(cnt >= k) return get_kth(wavelet[id].lid, k, wavelet[id].cnt[l - 1] + 1, wavelet[id].cnt[r]);
	return get_kth(wavelet[id].rid, k - cnt, l - wavelet[id].cnt[l - 1], r - wavelet[id].cnt[r]);
}

int cnt_k(int id, int k, int l, int r) {
	if(id == -1 || l > r || wavelet[id].mx <= k) return 0;
	if(wavelet[id].mn > k) return r - l + 1;
	return cnt_k(wavelet[id].lid, k, wavelet[id].cnt[l - 1] + 1, wavelet[id].cnt[r]) + 
		   cnt_k(wavelet[id].rid, k, l - wavelet[id].cnt[l - 1], r - wavelet[id].cnt[r]);
}